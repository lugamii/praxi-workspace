		...
		if (getParticipant(player) != null) {
			if (state == MatchState.STARTING_ROUND || state == MatchState.PLAYING_ROUND) {
				if (participantA.getPlayers().size() == 1 && participantB.getPlayers().size() == 1) {
					...
					lines.add("&cDuration: &f" + getDuration());
					lines.add("&cOpponent: &f" + opponent.getConjoinedNames());
				} else {
					...
					if (friendly.getPlayers().size() + opponent.getPlayers().size() <= 6) {
						...
						lines.add("&cDuration: &f" + getDuration());
						lines.add("&aTeam: &f" + friendly.getAliveCount() + "/" + friendly.getPlayers().size());
						lines.add("&cOpponents: &f" + opponent.getAliveCount() + "/" + opponent.getPlayers().size());
					}
				}
			} else {
				lines.add("&cDuration: &f" + TimeUtil.millisToTimer(timeData));
			}
		} else {
			lines.add("&cKit: &f" + getKit().getName());
			lines.add("&cDuration: &f" + getDuration());
			lines.add("");

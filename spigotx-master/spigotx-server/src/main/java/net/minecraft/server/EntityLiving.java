...
public abstract class EntityLiving extends Entity {
    ...
    public void a(Entity entity, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            KnockbackProfile kb = (this.getKnockbackProfile() == null) ? SpigotX.INSTANCE.getConfig().getCurrentKb() : this.getKnockbackProfile();


            final float magnitude = MathHelper.sqrt(d0 * d0 + d1 * d1);
            this.motX /= kb.getFriction();
            this.motY /= kb.getFriction();
            this.motZ /= kb.getFriction();
            this.motX -= d0 / magnitude * kb.getHorizontal();
            this.motY += kb.getVertical();
            this.motZ -= d1 / magnitude * kb.getHorizontal();
            if (this.motY > kb.getVerticalLimit()) {
                this.motY = kb.getVerticalLimit();
            }
        } else {
